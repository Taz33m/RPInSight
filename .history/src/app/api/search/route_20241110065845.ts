...
export async function POST(request: Request) {
  try {
    ...
    // If it's a nearest query and we have user location, find the nearest matching location
    if (isNearestQuery && userLocation) {
      // Filter locations based on the type of place they're looking for
      const searchTerm = query.toLowerCase();
      const relevantLocations = Object.entries(KNOWN_LOCATIONS).filter(([name]) => {
        if (searchTerm.includes('dining') || searchTerm.includes('food') || searchTerm.includes('lunch')) {
      ...
      return NextResponse.json({
        content: `${nearest.name.charAt(0).toUpperCase() + nearest.name.slice(1)} is the nearest location to you. It's approximately ${Math.round(nearest.distance)} meters from your current location.`,
        buildingName: nearest.name,
        coordinates: nearest.coordinates
      });
    }

    // If not a nearest query or no user location, proceed with normal AI response
